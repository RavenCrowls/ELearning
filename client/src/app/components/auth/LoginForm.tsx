...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_USER_API || 'http://localhost:5000'}/api/users/login`, {
        method: 'POST',
        ...
        <div className="w-full md:w-1/2 flex items-center justify-center p-8">
          <div className="w-full max-w-md">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex justify-end mb-6 cursor-pointer">
                <Link href="/forgot-password">
                  <button type="button" className="text-sm text-gray-600 hover:text-blue-600 cursor-pointer">
                    Quên mật khẩu
                  </button>
                </Link>
              </div>
            ...
            <div className="mt-6">
              <button
                type="button"
                className="w-full cursor-pointer flex items-center justify-center border border-[#4069E5] font-bold text-[#4069E5] py-2 rounded-md hover:bg-gray-50 transition duration-200"
              >
