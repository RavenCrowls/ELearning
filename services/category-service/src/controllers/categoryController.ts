...
class CategoryController {
  async createCategory(req: Request, res: Response) {
    try {
      const { CATEGORY_ID, NAME, STATUS, SUB_CATEGORIES } = req.body;
  ...
  async getAllCategories(req: Request, res: Response) {
    try {
      const categoryList = await Category.find({ STATUS: 1 })
        .select("-__v")
        .lean(); // Categories rarely change, safe to use lean()

      // Add cache headers since categories don't change often
