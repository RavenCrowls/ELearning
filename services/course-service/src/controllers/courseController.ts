...
class CourseController {
  ...
  async getAllCourses(req: Request, res: Response) {
    try {
      // Add pagination to prevent loading all courses at once
      ...
      const [courseList, total] = await Promise.all([
        ...
        Course.countDocuments({ STATUS: 1 }),
      ]);
  ...
  async getPopularCourse(req: Request, res: Response) {
    try {
      ...
      const popularCourses = await Course.find({ STATUS: 1 })
        ...
        .limit(limit)
        .select("-__v -DESCRIPTION -OUTPUT") // Exclude heavy fields for list view
  ...
  async searchCourseByName(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error searching courses by name", error });
    }
  }
  ...
  async getCoursesByInstructor(req: Request, res: Response) {
    try {
      ...
      if (!instructorId) {
        return res
          .status(400)
          .json({ message: "Missing instructor ID parameter" });
      }
      ...
      res
        .status(500)
        .json({ message: "Error getting courses by instructor", error });
    }
  }
}
